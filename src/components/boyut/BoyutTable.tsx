//src/components/boyut/BoyutTable.tsx
...
export type BoyutTranslation = {
  id: number | string;                                  // <-- EKLENDİ
  language: string;
  name: string;
  shortName: string;
...
export type BoyutRow = {
  ...
  status: StatusNum | "Aktif" | "Taslak" | "Pasif";
  date?: string;
  translations?: BoyutTranslation[];
};

type Actions = {
  onView: (row: BoyutRow) => void;
  onEdit: (row: BoyutRow) => void;
  onDelete: (row: BoyutRow) => void;
  onActivate: (row: BoyutRow) => void;
  onTranslate: (row: BoyutRow) => void;

  onTransView?: (parent: BoyutRow, t: BoyutTranslation) => void;
  onTransEdit?: (parent: BoyutRow, t: BoyutTranslation) => void;
  onTransActivate?: (parent: BoyutRow, t: BoyutTranslation) => void;
  onTransDelete?: (parent: BoyutRow, t: BoyutTranslation) => void;

  /** Yeni: çeviri satırı seçildiğinde */
  onSelectTranslation?: (parent: BoyutRow, t: BoyutTranslation) => void;
  /** Eski kullanım geriye uyum */
  onSelectLanguage?: (parent: BoyutRow, language: string) => void;
};

type Props = {
  rows: BoyutRow[];
  loading?: boolean;
  onRowSelect?: (row: BoyutRow) => void;
  selectedRowKey?: string;
  actions: Actions;
};

export default function BoyutTable({ rows, loading, onRowSelect, selectedRowKey, actions }: Props) {
  ...
  // DataTable'ın expandedKeys'ini güncelle
  ...
  return (
    <div className="space-y-2">
      ...
      <DataTable<BoyutRow>
        ...
        rowKey={(r) => `root-${r.id}`}
        columns={[
          { key: "name", title: "BOYUT ADI", width: 280, cellClassName: "cursor-pointer" },
  ...
  return (
    <div className="space-y-1">
      {items.map((t, idx) => (
          ...
          onClick={() => {
            actions.onSelectTranslation?.(parent, t);       // <-- çeviri seçim callback
            actions.onSelectLanguage?.(parent, t.language); // backward compatibility
          }}
        >
