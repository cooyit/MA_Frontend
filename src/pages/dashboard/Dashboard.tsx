...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors">
      <div className="mb-8">
        ...
        <Card title="Son İşlemler" icon={<TrendingUp className="w-6 h-6" />}>
          <div className="space-y-3">
            {recentActivities.map((activity, index) => (
              <div
                key={index}
                className="flex items-start gap-3 p-3 bg-gray-100 dark:bg-gray-800 rounded-lg"
              >
                <div
        ...
        <Card title="Sistem Durumu" icon={<Activity className="w-6 h-6" />}>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-800 dark:text-gray-100">Veritabanı</span>
              <StatusBadge status="active" label="Aktif" />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-800 dark:text-gray-100">API Servisleri</span>
              <StatusBadge status="error" label="Çalışmıyor" />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-800 dark:text-gray-100">Son Yedekleme</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">2 saat önce</span>
            </div>
          </div>
        </Card>
      </div>
